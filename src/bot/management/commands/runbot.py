...
class Command(BaseCommand):
    ...
    def handle_goals_list(self, msg: Message, tg_user: TgUser):
        ...
        if resp_goals:
            ...
            self.tg_client.send_message(msg.chat.id, '[Нет подходящей цели]')
    ...
    def handle_goals_category_list(self, msg: Message, tg_user: TgUser):
        ...
        if resp_categories:
            ...
            self.tg_client.send_message(msg.chat.id, '[Нет подходящей категории]')
    ...
    def handle_save_selected_category(self, msg: Message, tg_user: TgUser):
        if msg.text.isdigit():
            ...
            if GoalCategory.objects.filter(
                ...
                self.tg_client.send_message(msg.chat.id, '[Введите заголовок]')
                self.storage.set_state(msg.chat.id, state=StateEnum.CHOSEN_CATEGORY)
            elif GoalCategory.objects.filter(
                    board__participants__user_id=tg_user.user_id,
                    board__participants__role=BoardParticipant.Role.reader,
                    is_deleted=False,
                    id=cat_id).exists():
                self.tg_client.send_message(msg.chat.id, '[Нет полномочий на данную категорию]')
            else:
                self.tg_client.send_message(msg.chat.id, '[Нет подходящей категории]')

        else:
            self.tg_client.send_message(msg.chat.id, '[Некорректная категория]')
    ...
    def handle_save_new_cat(self, msg: Message, tg_user: TgUser):
        ...
        if goal.is_completed:
            ...
            self.tg_client.send_message(msg.chat.id, '[Ошибка]')
    ...
    def handle_verified_user(self, msg: Message, tg_user: TgUser):
        if msg.text == '/info':
            ...
            self.tg_client.send_message(msg.chat.id, '[Операция отменена]')
            ...
            self.tg_client.send_message(msg.chat.id, '[Команда неизвестна]')
